...
class Environment(object):
    ...
    def __init__(self, verbose=False, num_dummies=100, grid_size = (8, 6), epsilon_func=None, alpha_func=None):
        ...
        for agent in self.agent_states.iterkeys():
            ...
            if agent is self.primary_agent:
                agent.reset(destination=(destination if agent is self.primary_agent else None), testing=testing,
                            epsilon_func=self.epsilon_func, alpha_func=self.alpha_func)
            else:
                agent.reset(destination=(destination if agent is self.primary_agent else None), testing=testing)
