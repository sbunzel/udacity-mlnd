...
class Simulator(object):
    ...
    def __init__(self, env, size=None, update_delay=2.0, display=True, log_metrics=False, optimized=False,
                 epsilon_desc="not_set", alpha_desc="not_set"):
        self.env = env
        ...
        if self.log_metrics:
            ...
            if a.learning:
                if self.optimized: # Whether the user is optimizing the parameters and decay functions
                    self.log_filename = os.path.join("logs", "sim_improved-learning_epsilon_" + epsilon_desc + "_alpha_" + alpha_desc + ".csv")
                    self.table_filename = os.path.join("logs","sim_improved-learning_epsilon_" + epsilon_desc + "_alpha_" + alpha_desc + ".txt")
                else: 
